...
from AniAlert.cogs.cogs.search_image import SearchAnimeByImage


async def setup(bot):
  await bot.add_cog(SeasonalAnimeLookUpCog(bot))
  await bot.add_cog(AllAnimeSearchCog(bot))
  await bot.add_cog(CheckNotifyListCog(bot))
  await bot.add_cog(NotifyAnimeAiredCog(bot))
  await bot.add_cog(RemoveAnimeCog(bot))
  await bot.add_cog(ClearNotifyListCog(bot))
